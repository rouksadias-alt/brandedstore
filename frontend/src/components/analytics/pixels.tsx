...
// Set these in .env.local / Easypanel to turn on ad tracking. All three are
// no-ops (nothing rendered) if left empty — see frontend/.env.example.
//
// strategy="afterInteractive" is Next.js's recommended deferral for
// analytics/pixel scripts: they're excluded from the initial HTML and only
...
export function Pixels() {
  return (
    <>
      ...
      {FB_PIXEL_ID && (
        // eslint-disable-next-line @next/next/no-img-element -- required literally by Meta's own snippet
        <noscript>
          <img
            height="1"
            width="1"
      ...
      {SNAPCHAT_PIXEL_ID && (
        <Script id="snapchat-pixel-base" strategy="afterInteractive">
          {`
