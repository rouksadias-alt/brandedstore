...
import { BUSINESS, getProductBySlug, type DuoOffer } from "@/lib/products";
import { formatUSD } from "@/lib/utils";

export function DuoLandingPage({ duo }: { duo: DuoOffer }) {
  const savings = duo.compareAtPrice - duo.price;
  const items = duo.productSlugs.map((slug) => getProductBySlug(slug)).filter(Boolean);
  const checkoutHref = `/checkout?product=${duo.checkoutProductSlug}&plan=${duo.checkoutPlanId}`;
    ...
    <>
      ...
      <Section id="hero" className="pb-10 pt-8 sm:pt-14">
        <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
              ...
              <div className="flex flex-col gap-0.5">
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-base text-ink/50 line-through">
                    {formatUSD(duo.compareAtPrice)}
                  </span>
                </div>
              </div>
            </div>
            <p className="mt-2 text-sm font-bold text-mint-700">
              Ahorras {formatUSD(savings)} vs. comprar por separado
            </p>
          ...
          <ProductVisual emoji={duo.emoji} gradient={duo.gradient} images={duo.images} />
        </div>
      </Section>

      {/* Qué incluye */}
      <Section className="bg-mint-50/50">
        <SectionHeading eyebrow="Qué incluye" title="Los 2 productos, un dúo completo" />
      ...
      <Section id="oferta">
        <div className="mx-auto max-w-xl rounded-3xl border-2 border-mint-600 bg-mint-50 p-6 sm:p-8 text-center shadow-lg">
          ...
          <p className="mt-4 font-display text-2xl font-semibold text-ink">{duo.title}</p>
          <div className="mt-3 flex items-center justify-center gap-2">
            <span className="text-4xl font-extrabold text-mint-700">{formatUSD(duo.price)}</span>
            <span className="text-lg text-ink/50 line-through">{formatUSD(duo.compareAtPrice)}</span>
          </div>
          <p className="mt-1 text-sm font-semibold text-mint-700">Ahorras {formatUSD(savings)}</p>
          ...
          <LinkButton href={checkoutHref} size="lg" className="mt-7 w-full">
            Pedir el Dúo
          </LinkButton>
      ...
      <Section>
        <div className="mx-auto max-w-3xl rounded-3xl bg-mint-900 px-6 py-14 text-center text-white sm:px-14">
          <h2 className="font-display text-3xl font-semibold sm:text-4xl">{duo.tagline}</h2>
